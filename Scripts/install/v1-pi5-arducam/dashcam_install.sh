...
# Configuration
INSTALL_DIR="/home/$SUDO_USER/dashcam"
VIDEO_DIR="/dashcam/videos"
LOG_DIR="/dashcam/logs"
...
chown -R $SUDO_USER:$SUDO_USER /dashcam
...
su - $SUDO_USER -c "python3 -m venv --system-site-packages $VENV_DIR"

# Install Python packages in venv
echo -e "${BLUE}Installing Python packages in virtual environment...${NC}"
su - $SUDO_USER -c "$VENV_DIR/bin/pip install --upgrade pip"
su - $SUDO_USER -c "$VENV_DIR/bin/pip install \
    pillow \
    gps \
    pyserial \
    pyyaml \
...
echo ""
echo -e "${GREEN}Step 7: Configuring GPS (GPSD)...${NC}"
...
echo ""
echo -e "${GREEN}Step 8: Configuring CAN bus...${NC}"
...
After=network.target graphical.target gpsd.service
Wants=gpsd.service
...
ExecStart=$VENV_DIR/bin/python $INSTALL_DIR/dashcam.py
Restart=always
RestartSec=5
...
echo ""
echo -e "${GREEN}Step 10: Disabling screen blanking...${NC}"
...
echo ""
echo -e "${GREEN}Step 11: Configuring framebuffer permissions...${NC}"
...
echo ""
echo -e "${GREEN}Step 12: Setting boot order for NVMe...${NC}"
...
echo ""
echo -e "${GREEN}Step 13: Creating camera test script...${NC}"

cat > "$INSTALL_DIR/test_cameras.sh" << 'TESTEOF'
#!/bin/bash
...
chmod +x "$INSTALL_DIR/test_cameras.sh"
chown $SUDO_USER:$SUDO_USER "$INSTALL_DIR/test_cameras.sh"

echo -e "${GREEN}✓ Camera test script created: $INSTALL_DIR/test_cameras.sh${NC}"
...
echo "     $INSTALL_DIR/test_cameras.sh"
echo "     OR manually:"
echo "     rpicam-hello --list-cameras"
echo "     rpicam-hello --camera 0  # Test front camera"
echo "     rpicam-hello --camera 1  # Test rear camera"
echo ""
echo "  3. Check GPS:"
echo "     cgps -s"
echo "     gpsmon"
echo ""
echo "  4. Check CAN bus:"
echo "     ip link show can0"
echo "     ip link show can1"
echo "     candump can0  # Monitor CAN traffic"
echo ""
